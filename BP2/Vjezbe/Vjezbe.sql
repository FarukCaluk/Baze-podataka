...
(
	AutoriID varchar(11) constraint PK_Autori primary key,
...
(
	KnjigeID nvarchar(6) constraint PK_Knjige primary key,
	Naziv nvarchar(80) NOT NULL,
	Vrsta char(12) NOT NULL,
	IzdavacID char(4),
	Cijena money,
	Bijeska nvarchar(200),
	Datum datetime
)
...
--5. U kreiranoj tabeli Izdavaci provjeriti koje polje je primarni ključ, ukoliko ga nema, prikladno polje
--proglasiti primarnim ključem
...
(
	AutorID varchar(11) constraint FK_AutoriKnjige_Autori foreign key references Prodaja.Autori(AutoriID),
	KnjigaID nvarchar(6) constraint FK_AutoriKnjige_Knjige foreign key references Prodaja.Knjige(KnjigeID),
	AuOrd tinyint,
...
select t.title_id,t.title,t.type,t.pub_id,t.price,isnull(t.notes,'nepoznata vrijednost') as 'notes' ,t.pubdate
from pubs.dbo.titles as t

insert into Prodaja.Autori
select a.au_id,a.au_lname,a.au_lname,a.phone,a.address,a.state,a.zip,a.contract
...
where upper(a.Ime) like 'D%' or upper(a.Ime) like 'S%' 
...
--12. U tabeli Izdavaci nad poljem NazivIzdavaca postaviti ograničenje kojim se onemogućuje unos
--duplikata
...
values('SASX' , 'ĐidoMova')

--14. U bazi Vjezba2 kreirati šemu Narudzbe

go
create schema Narudzba
go
...
select*
into Narudzba.StavkeNarudzbe 
from Northwind.dbo.[Order Details]

--20. U tabeli StavkeNarudzbe dodati standardnu kolonu ukupno tipa decimalni broj (8,2).

alter table Narudzba.StavkeNarudzbe 
add Ukupno decimal(12, 2)

--21. Izvršiti update kreirane kolone kao umnožak kolona Quantity i UnitPrice.

update Narudzba.StavkeNarudzbe
set Ukupno = Quantity * UnitPrice
--XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
alter table Narudzba.StavkeNarudzbe
add UkupnoCijena as ((Quantity * UnitPrice) * (1-Discount)) 
--22. U tabeli StavkeNarduzbe dodati izračunatu kolonu CijeliDio(broj ispred decimalnog zareza) u kojoj
--će biti cijeli dio iz kolone UnitPrice
alter table Narudzba.StavkeNarudzbe
add CijeliBroj as floor(UnitPrice)
--23. U tabeli StavkeNarduzbe kreirati ograničenje na koloni Discount kojim će se onemogućiti unos
--vrijednosti manjih od 0.
alter table Narudzba.StavkeNarudzbe
add constraint CK_StavkeNarudzbe_Discount check(Discount >= 0 and Discount <= 1)
--add constraint CK_StavkeNarudzbe_Discount check(between 0 and 1)
--24. U tabelu StavkeNarudzbe insertovati novi zapis (potrebno je testirati postavljeno ograničenje)
insert into Narudzba.StavkeNarudzbe(OrderID,ProductID,UnitPrice,Quantity,Discount)
values (23,22,58,4,2)
...
create table Narudzba.Proizvodi
(
	ProizvoidID int constraint PK_Proizvodi primary key identity(1,1),
...
--2. Iz baze podataka AdventureWorks2017 u tabelu Proizvodi dodati sve proizvode, na mjestima gdje nema
--pohranjenih podataka o težini zamijeniti vrijednost sa 0
...
--• Weight -> Tezina
set identity_insert Narudzba.Proizvodi ON
insert into Narudzba.Proizvodi(ProizvoidID,Naziv,SifraProizvoda,Boja,NazivKategorije,Tezina)
select p.ProductID,p.Name,p.ProductNumber,p.Color,p.Name, isnull(p.Weight,0)
from AdventureWorks2017.Production.Product as p
	...
	--INNER JOIN AdventureWorks2017.Production.ProductCategory as pc ON p.ProductCategoryID = pc.ProductCategoryID
set identity_insert Narudzba.Proizvodi OFF

select * from Narudzba.Proizvodi
